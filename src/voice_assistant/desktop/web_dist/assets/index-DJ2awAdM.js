(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))a(s);new MutationObserver(s=>{for(const o of s)if(o.type==="childList")for(const n of o.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&a(n)}).observe(document,{childList:!0,subtree:!0});function t(s){const o={};return s.integrity&&(o.integrity=s.integrity),s.referrerPolicy&&(o.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?o.credentials="include":s.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function a(s){if(s.ep)return;s.ep=!0;const o=t(s);fetch(s.href,o)}})();class G{el;constructor(e){if(this.el=document.createElement("div"),this.el.className="va-bg",this.el.setAttribute("aria-hidden","true"),this.el.innerHTML=`
    ...
    `,e.insertBefore(this.el,e.firstChild),!document.querySelector("#va-bg-style")){const t=document.createElement("style");t.id="va-bg-style",t.textContent=`
      ...
      `,document.head.appendChild(t)}}destroy(){this.el.remove()}}class X{el;constructor(e){this.el=document.createElement("header"),this.el.className="h-12 px-5 flex items-center justify-between border-b border-border bg-surface/40",this.el.innerHTML=`
    ...
    `,e.appendChild(this.el)}setStatus(e){const t=this.el.querySelector("[data-status]");t&&(t.textContent=e)}onSettingsClick(e){this.el.querySelector("[data-settings]").addEventListener("click",e)}}const T={aria:"avatars/aria.svg",liam:"avatars/liam.svg",sage:"avatars/sage.svg"};class J{el;img;halo;dots;constructor(e,t="aria"){this.el=document.createElement("div"),this.el.className="flex-1 grid place-items-center relative",this.el.innerHTML=`
    ...
    `,document.head.appendChild(e)}}class re{el;input;micBtn;sendBtn;submitHandler=()=>{};recordHandler=()=>{};pendingImages=[];constructor(e){this.el=document.createElement("footer"),this.el.className="va-composer",this.el.innerHTML=`
      <div class="va-composer-image-strip hidden" data-image-strip aria-label="${r("composer.images_label")}"></div>
      <div class="va-composer-inner glass">
        <button data-record type="button" aria-label="${r("composer.start_listening")}" class="va-mic-btn" title="${r("composer.start_listening")}">
        ...
        <textarea data-input rows="1" placeholder="${r("composer.placeholder")}"
          class="va-composer-input" aria-label="${r("composer.placeholder")}"></textarea>
        <button data-send type="button" aria-label="${r("composer.send")}" class="va-send-btn hidden" title="${r("composer.send")}">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
    ...
    `,e.appendChild(this.el),this.input=this.el.querySelector("[data-input]"),this.micBtn=this.el.querySelector("[data-record]"),this.sendBtn=this.el.querySelector("[data-send]"),this._wireEvents(),this._injectStyles()}onSubmit(e){this.submitHandler=e}onRecord(e){this.recordHandler=e}setListening(e){const t=this.el.querySelector("[data-mic-icon]"),a=this.el.querySelector("[data-listen-dot]");t.classList.toggle("hidden",e),a.classList.toggle("hidden",!e),this.micBtn.setAttribute("aria-label",r(e?"composer.recording":"composer.start_listening"))}_wireEvents(){this.input.addEventListener("input",()=>{this._resize(),this._toggleSend()}),this.input.addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),this._submit())}),this.sendBtn.addEventListener("click",()=>this._submit()),this.micBtn.addEventListener("click",()=>this.recordHandler()),this.el.addEventListener("paste",e=>{const t=e.clipboardData?.items;if(t){for(const a of Array.from(t))if(a.type.startsWith("image/")){e.preventDefault();const s=a.getAsFile();s&&this._addImageFile(s)}}}),this.el.addEventListener("dragover",e=>{e.dataTransfer?.types.includes("Files")&&(e.preventDefault(),this.el.classList.add("va-composer--drag"))}),this.el.addEventListener("dragleave",()=>{this.el.classList.remove("va-composer--drag")}),this.el.addEventListener("drop",e=>{e.preventDefault(),this.el.classList.remove("va-composer--drag");const t=e.dataTransfer?.files;if(t)for(const a of Array.from(t))a.type.startsWith("image/")&&this._addImageFile(a)})}_submit(){const e=this.input.value.trim();!e&&this.pendingImages.length===0||(this.submitHandler({text:e,images:this.pendingImages}),this.pendingImages=[],this._refreshImageStrip(),this.input.value="",this._resize(),this._toggleSend())}async _addImageFile(e){const t=await this._fileToDataUrl(e);this.pendingImages.push(t),this._refreshImageStrip(),this._toggleSend()}_fileToDataUrl(e){return new Promise((t,a)=>{const s=new FileReader;s.onload=()=>t(s.result),s.onerror=a,s.readAsDataURL(e)})}_refreshImageStrip(){const e=this.el.querySelector("[data-image-strip]");if(this.pendingImages.length===0){e.innerHTML="",e.classList.add("hidden");return}e.classList.remove("hidden"),e.innerHTML=this.pendingImages.map((t,a)=>`
      <div class="va-img-thumb" data-idx="${a}">
        <img src="${t}" alt="${r("composer.image_thumbnail")} ${a+1}" class="va-img-thumb-img" />
        <button type="button" class="va-img-thumb-remove" aria-label="${r("composer.remove_image")} ${a+1}" data-remove="${a}">×</button>
      </div>
    `).join(""),e.querySelectorAll("[data-remove]").forEach(t=>{t.addEventListener("click",()=>{const a=parseInt(t.dataset.remove??"0",10);this.pendingImages.splice(a,1),this._refreshImageStrip(),this._toggleSend()})})}_resize(){this.input.style.height="auto";const t=24*4+16;this.input.style.height=Math.min(this.input.scrollHeight,t)+"px"}_toggleSend(){const e=this.input.value.trim().length>0||this.pendingImages.length>0;this.sendBtn.classList.toggle("hidden",!e)}_injectStyles(){if(document.querySelector("#va-composer-style"))return;const e=document.createElement("style");e.id="va-composer-style",e.textContent=`
      .va-composer {
      ...
      <div class="va-settings-panel">
        ...
        <div class="va-settings-body">
          ...
          <div class="va-tab-panels">
            ${I.map(t=>`<div role="tabpanel" id="va-tab-${t.id}" data-panel="${t.id}" class="va-tab-panel${t.id===this.activeTab?" is-active":""}" aria-labelledby="va-tab-btn-${t.id}"></div>`).join("")}
          </div>
        </div>
    ...
    `}_buildAudioPanel(){const e=this._panel("audio");e.innerHTML=`
      ...
      <div data-wake-row class="va-field hidden">
        ...
        <select data-field="wake_word" class="va-select">
          ${le.map(s=>`<option value="${s}">${s.replace(/_/g," ")}</option>`).join("")}
        </select>
      </div>
    ...
    `,e.querySelector('[data-field="user_address_as"]').addEventListener("change",()=>{this._refreshTitleRow(e)})}_buildPrivacyPanel(){const e=this._panel("privacy");e.innerHTML=`
      <div class="va-field">
        <label class="va-label">${r("allowed_folders.label")}</label>
        <input data-field="roots" class="va-input" placeholder="~" />
        <p class="va-hint">${r("allowed_folders.hint")}</p>
      </div>
      <!-- Display group -->
      <div class="va-section-divider"></div>
      <p class="va-section-heading">${r("display.group")}</p>
      <div class="va-field">
        <label class="va-label va-label--checkbox">
          <input data-field="display_hc" type="checkbox" />
          ${r("display.high_contrast")}
        </label>
      </div>
      <div class="va-field">
        <label class="va-label" for="display-font-size">${r("display.font_size")}</label>
        <select data-field="display_font_size" id="display-font-size" class="va-select">
          <option value="small">${r("display.font_small")}</option>
          <option value="medium" selected>${r("display.font_medium")}</option>
          <option value="large">${r("display.font_large")}</option>
          <option value="xl">${r("display.font_xl")}</option>
        </select>
      </div>
      <!-- Onboarding -->
      <div class="va-section-divider"></div>
      <div class="va-field">
        <button type="button" data-show-onboarding class="va-link-btn">${r("onboarding.show_again")}</button>
      </div>
    `,e.querySelector("[data-show-onboarding]").addEventListener("click",()=>{this.close(),this.showOnboardingHandler&&this.showOnboardingHandler()})}_populate(e){this.currentCfg=e;const t=this._panel("brain");t.querySelector('[data-field="provider"]').value=e.provider,t.querySelector('[data-field="respond_in"]').value=e.respond_in||"auto",this._refreshModelOptions(),this._refreshSecretFields(),this._refreshVoiceSection();const a=this._panel("audio"),s=e.audio_trigger||"hotkey";a.querySelectorAll('[name="audio_trigger"]').forEach(g=>{g.checked=g.value===s}),a.querySelector('[data-field="hotkey"]').value=e.hotkey,a.querySelector('[data-field="wake_word"]').value=e.wake_word||"hey_jarvis";const o=e.wake_sensitivity??.5,n=a.querySelector('[data-field="wake_sensitivity"]');n.value=String(o),a.querySelector("[data-sensitivity-val]").textContent=String(o),this._refreshAudioTrigger(a);const p=this._panel("identity");p.querySelector('[data-field="user_name"]').value=e.user_name||"",p.querySelector('[data-field="user_address_as"]').value=e.user_address_as||"none",p.querySelector('[data-field="user_title"]').value=e.user_title||"",this._refreshTitleRow(p);const h=this._panel("privacy");h.querySelector('[data-field="roots"]').value=e.allowed_roots.join(", "),h.querySelector('[data-field="display_hc"]').checked=e.display_theme==="hc",h.querySelector('[data-field="display_font_size"]').value=e.display_font_size??"medium";const u=this.el.querySelector("[data-ai-footer]"),l=e.provider,d=de[l]??"#";u.innerHTML=`AI assistant powered by <strong>${l}</strong>. Inputs are sent to <a href="${d}" target="_blank" rel="noopener noreferrer" class="va-ai-footer-link">${l} privacy policy</a>.`}_refreshAudioTrigger(e){const t=e.querySelector('[name="audio_trigger"]:checked')?.value||"hotkey";e.querySelector("[data-hotkey-row]").classList.toggle("hidden",t!=="hotkey"),e.querySelector("[data-wake-row]").classList.toggle("hidden",t!=="wake_word"),e.querySelector("[data-sensitivity-row]").classList.toggle("hidden",t!=="wake_word")}_refreshTitleRow(e){const t=e.querySelector('[data-field="user_address_as"]').value;e.querySelector("[data-title-row]").classList.toggle("hidden",t!=="title")}_refreshModelOptions(){const e=this._panel("brain"),t=e.querySelector('[data-field="provider"]').value,a=e.querySelector('[data-field="model"]'),s=this.currentCfg?.available_models?.[t]??[],o=this.currentCfg?.model;a.innerHTML="";const n=new Set;for(const p of s)n.add(p),a.appendChild(this._opt(p));o&&!n.has(o)&&a.appendChild(this._opt(o+" (custom)",o)),o&&(a.value=o)}_refreshSecretFields(){const e=this._panel("brain"),t=e.querySelector('[data-field="provider"]').value,a=e.querySelector("[data-secret-label]"),s=e.querySelector('[data-field="secret"]'),o=e.querySelector("[data-secret-hint]"),n=e.querySelector("[data-saved-badge]"),p=e.querySelector("[data-eye-show]"),h=e.querySelector("[data-eye-hide]");a.textContent=t==="ollama"?"Ollama base URL":"API key",s.value="",s.type=t==="ollama"?"text":"password",p.classList.remove("hidden"),h.classList.add("hidden");const u=t===this.currentCfg?.provider&&this.currentCfg?.has_secret===!0;n.classList.toggle("hidden",!u),t==="ollama"?(s.placeholder=this.currentCfg?.ollama_base_url||"http://localhost:11434",o.textContent="URL of your local Ollama instance."):u?(s.placeholder="✓ key on file — leave blank to keep current",o.textContent="Stored in ~/.voice-assistant/.env, mode 0600. Type a new key only to replace it."):(s.placeholder=t==="anthropic"?"sk-ant-…":t==="openai"?"sk-…":"AIza…",o.textContent="Stored in ~/.voice-assistant/.env, mode 0600.")}_refreshVoiceSection(){const e=this._panel("voice"),t=this.currentCfg?.available_avatars??["aria","liam","sage"],a=e.querySelector('[data-field="avatar"]');a.innerHTML="";for(const l of t){const d=document.createElement("button");d.type="button",d.className="va-avatar-btn",d.dataset.avatar=l;const g=l===(this.currentCfg?.avatar??"aria");d.setAttribute("aria-pressed",String(g)),g&&d.classList.add("is-selected"),d.innerHTML=`<img src="avatars/${l}.svg" alt="${l}" class="va-avatar-img" /><p class="va-avatar-name">${l}</p>`,d.addEventListener("click",()=>{a.querySelectorAll(".va-avatar-btn").forEach(m=>{m.setAttribute("aria-pressed","false"),m.classList.remove("is-selected")}),d.setAttribute("aria-pressed","true"),d.classList.add("is-selected")}),a.appendChild(d)}const s=this.currentCfg?.available_voices??[],o=e.querySelector('[data-field="voice"]');o.innerHTML="";const n={};for(const l of s)(n[l.language]??=[]).push(l);const p=Object.keys(n).sort((l,d)=>l==="en"?-1:d==="en"?1:l.localeCompare(d));for(const l of p){const d=document.createElement("optgroup"),m=n[l][0].label.match(/\(([^)]+)\)/);d.label=m?m[1]:l==="en"?"English":l;for(const f of n[l])d.appendChild(this._opt(f.notes?`${f.label} · ${f.notes}`:f.label,f.id));o.appendChild(d)}this.currentCfg?.voice&&(o.value=this.currentCfg.voice);const h=this.currentCfg?.available_stt_languages??[],u=e.querySelector('[data-field="stt_language"]');u.innerHTML="";for(const l of h)u.appendChild(this._opt(l.label,l.code));this.currentCfg?.stt_language&&(u.value=this.currentCfg.stt_language)}_read(){const e=this._panel("brain"),t=this._panel("voice"),a=this._panel("audio"),s=this._panel("identity"),o=this._panel("privacy"),n=e.querySelector('[data-field="provider"]').value,p=e.querySelector('[data-field="model"]').value.trim(),h=e.querySelector('[data-field="secret"]').value.trim(),u=e.querySelector('[data-field="respond_in"]').value,l=t.querySelector('[data-field="voice"]')?.value||this.currentCfg?.voice||"piper:en_US-amy-medium",d=t.querySelector('[data-field="stt_language"]')?.value||this.currentCfg?.stt_language||"auto",m=t.querySelector('[data-field="avatar"] [aria-pressed="true"]')?.dataset.avatar??this.currentCfg?.avatar??"aria",R=a.querySelector('[name="audio_trigger"]:checked')?.value||"hotkey",j=a.querySelector('[data-field="hotkey"]').value.trim()||"ctrl+shift+space",N=a.querySelector('[data-field="wake_word"]').value||"hey_jarvis",D=parseFloat(a.querySelector('[data-field="wake_sensitivity"]').value||"0.5"),O=s.querySelector('[data-field="user_name"]').value.trim()||null,K=s.querySelector('[data-field="user_address_as"]').value,F=s.querySelector('[data-field="user_title"]').value.trim()||null,V=o.querySelector('[data-field="roots"]').value.split(",").map(Y=>Y.trim()).filter(Boolean),U=o.querySelector('[data-field="display_hc"]').checked,W=o.querySelector('[data-field="display_font_size"]').value,A={provider:n,model:p,hotkey:j,allowed_roots:V,ollama_base_url:n==="ollama"?h||this.currentCfg?.ollama_base_url||"http://localhost:11434":null,user_name:O,user_address_as:K,user_title:F,respond_in:u,voice:l,stt_language:d,avatar:m,audio_trigger:R,wake_word:N,wake_sensitivity:D,display_theme:U?"hc":"default",display_font_size:W??"medium",locale:this.currentCfg?.locale,transparency_acknowledged:this.currentCfg?.transparency_acknowledged};return n!=="ollama"&&h&&(A._secret=h),A}_panel(e){return this.el.querySelector(`[data-panel="${e}"]`)}_opt(e,t){const a=document.createElement("option");return a.textContent=e,a.value=t??e,a}_injectStyles(){if(document.querySelector("#va-settings-style"))return;const e=document.createElement("style");e.id="va-settings-style",e.textContent=`
      .va-settings {
        position: fixed;
        inset: 0;
        z-index: 40;
    ...
    `,e.appendChild(this.el),this._injectStyles()}show(e,t){if(this._visible)return;this._visible=!0;const a=this.el.querySelector("#va-transparency-title"),s=this.el.querySelector("#va-transparency-body"),o=this.el.querySelector("[data-ok]");a.textContent=r("transparency.title"),s.textContent=r("transparency.body"),o.textContent=r("transparency.ok"),this.el.classList.add("is-open"),document.body.style.overflow="hidden",o.focus();const n=async()=>{this.el.classList.remove("is-open"),document.body.style.overflow="",this._visible=!1,await t()};o.onclick=()=>{n()}}_injectStyles(){if(document.querySelector("#va-transparency-style"))return;const e=document.createElement("style");e.id="va-transparency-style",e.textContent=`
    ...
    `,document.head.appendChild(e)}}const x=[{titleKey:"onboarding.slide1.title",bodyKey:"onboarding.slide1.body",icon:`<svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
      <circle cx="12" cy="12" r="10"/>
      <path d="M12 8v4l3 3"/>
    </svg>`},{titleKey:"onboarding.slide2.title",bodyKey:"onboarding.slide2.body",icon:`<svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
      <circle cx="12" cy="12" r="5"/>
      <circle cx="12" cy="12" r="9" stroke-dasharray="3 3"/>
      <circle cx="12" cy="12" r="1" fill="currentColor"/>
    </svg>`},{titleKey:"onboarding.slide3.title",bodyKey:"onboarding.slide3.body",icon:`<svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
      <path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/>
    </svg>`},{titleKey:"onboarding.slide4.title",bodyKey:"onboarding.slide4.body",icon:`<svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
      <rect x="3" y="11" width="18" height="11" rx="2"/>
      <path d="M7 11V7a5 5 0 0 1 10 0v4"/>
    </svg>`}];class be{el;currentSlide=0;doneHandler=null;constructor(e){this.el=document.createElement("div"),this.el.className="va-onboarding",this.el.setAttribute("role","dialog"),this.el.setAttribute("aria-modal","true"),this.el.setAttribute("aria-label",r("onboarding.aria_label")),this.el.innerHTML=`
      <div class="va-onboarding-backdrop"></div>
      <div class="va-onboarding-panel">
        <button type="button" class="va-onboarding-skip" data-skip aria-label="${r("onboarding.skip")}">
          ${r("onboarding.skip")}
        </button>
        ...
        <div class="va-onboarding-footer">
          ...
          <div class="va-onboarding-btns">
            <button type="button" class="va-btn va-btn--ghost" data-prev>${r("onboarding.prev")}</button>
            <button type="button" class="va-btn va-btn--primary" data-next>${r("onboarding.next")}</button>
          </div>
        </div>
      </div>
    `,e.appendChild(this.el),this._renderSlides(),this._renderDots(),this._wireEvents(),this._injectStyles()}show(e){this.doneHandler=e,this.currentSlide=0,this._update(),this.el.classList.add("is-visible"),document.body.style.overflow="hidden",this.el.querySelector("[data-next]")?.focus()}hide(){this.el.classList.remove("is-visible"),document.body.style.overflow=""}_renderSlides(){const e=this.el.querySelector("[data-slides]");e.innerHTML=x.map((t,a)=>`
      <div class="va-onboarding-slide${a===0?" is-active":""}" data-slide="${a}" aria-hidden="${a!==0}"${a===0?"":" hidden"}>
        <div class="va-onboarding-icon">${t.icon}</div>
        <h2 class="va-onboarding-title">${r(t.titleKey)}</h2>
        <p class="va-onboarding-body">${r(t.bodyKey)}</p>
      </div>
    `).join("")}_renderDots(){const e=this.el.querySelector("[data-dots]");e.innerHTML=x.map((t,a)=>`
      <button type="button" class="va-onboarding-dot${a===0?" is-active":""}"
        data-dot="${a}" aria-label="${r("onboarding.go_to_slide")} ${a+1}">
      </button>
    `).join(""),e.querySelectorAll("[data-dot]").forEach(t=>{t.addEventListener("click",()=>{this.currentSlide=parseInt(t.dataset.dot??"0",10),this._update()})})}_wireEvents(){this.el.querySelector("[data-skip]").addEventListener("click",()=>this._finish()),this.el.querySelector("[data-prev]").addEventListener("click",()=>{this.currentSlide>0&&(this.currentSlide--,this._update())}),this.el.querySelector("[data-next]").addEventListener("click",()=>{this.currentSlide<x.length-1?(this.currentSlide++,this._update()):this._finish()}),this.el.querySelector(".va-onboarding-backdrop").addEventListener("click",()=>this._finish()),this.el.addEventListener("keydown",e=>{if(e.key==="Escape"){this._finish();return}(e.key==="ArrowRight"||e.key==="ArrowDown")&&this.currentSlide<x.length-1&&(this.currentSlide++,this._update()),(e.key==="ArrowLeft"||e.key==="ArrowUp")&&this.currentSlide>0&&(this.currentSlide--,this._update())})}_update(){const e=x.length,t=this.currentSlide;this.el.querySelectorAll("[data-slide]").forEach(o=>{const p=parseInt(o.dataset.slide??"0",10)===t;o.classList.toggle("is-active",p),o.setAttribute("aria-hidden",String(!p)),o.toggleAttribute("hidden",!p)}),this.el.querySelectorAll("[data-dot]").forEach(o=>{o.classList.toggle("is-active",parseInt(o.dataset.dot??"0",10)===t)});const a=this.el.querySelector("[data-prev]"),s=this.el.querySelector("[data-next]");a.disabled=t===0,s.textContent=t===e-1?r("onboarding.get_started"):r("onboarding.next"),s.dataset.last=t===e-1?"1":"0"}_finish(){this.hide(),this.doneHandler&&(this.doneHandler(),this.doneHandler=null)}_injectStyles(){if(document.querySelector("#va-onboarding-style"))return;const e=document.createElement("style");e.id="va-onboarding-style",e.textContent=`
      .va-onboarding {
        position: fixed;
        inset: 0;
        z-index: 900;
    ...
    `,document.head.appendChild(e)}}class ve{listeners=[];emit(e){for(const t of this.listeners)t(e)}on(e){return this.listeners.push(e),()=>{this.listeners=this.listeners.filter(t=>t!==e)}}}const w=new ve;window.va={emit:i=>w.emit(i)};const v=()=>typeof window<"u"&&typeof window.pywebview<"u",b={startListening:async()=>v()?window.pywebview.api.start_listening():void 0,stopListening:async()=>v()?window.pywebview.api.stop_listening():void 0,sendText:async(i,e)=>v()?window.pywebview.api.send_text(i,e):void 0,getConfig:async()=>v()?window.pywebview.api.get_config():{provider:"anthropic",model:"claude-sonnet-4-6",hotkey:"ctrl+shift+space",allowed_roots:["~"],ollama_base_url:null},saveConfig:async i=>v()?window.pywebview.api.save_config(i):{ok:!0},getHistory:async()=>v()?window.pywebview.api.get_history():[],quit:async()=>v()?window.pywebview.api.quit():void 0};v()?document.addEventListener("pywebviewready",()=>{b.getConfig().then(i=>w.emit({type:"config",cfg:i})),b.getHistory().then(i=>w.emit({type:"history_replay",items:i}))}):setTimeout(async()=>{const i=await b.getConfig();w.emit({type:"config",cfg:i})},100);const E=document.getElementById("app");E.className="va-app";E.innerHTML="";new G(document.body);const C=document.createElement("div");C.className="va-shell";E.appendChild(C);const ge=new he(C),y=document.createElement("div");y.className="va-main";C.appendChild(y);const q=new X(y),S=new J(y,"aria"),z=new oe(y),_=new se(y),M=new re(y),k=new ce(document.body),me=new pe(document.body),ye=new ue(document.body),P=new be(document.body);let c=null;function fe(i){const e=["ur","ar","he","fa"].includes(i);document.documentElement.dir=e?"rtl":"ltr",document.documentElement.lang=i||"en"}function xe(i){const e=document.documentElement;i.display_theme==="hc"?e.setAttribute("data-theme","hc"):e.removeAttribute("data-theme");const t={small:"87.5%",medium:"100%",large:"112.5%",xl:"125%"};e.style.fontSize=t[i.display_font_size??"medium"]??"100%"}(function(){if(document.querySelector("#va-layout-style"))return;const e=document.createElement("style");e.id="va-layout-style",e.textContent=`
    #app.va-app {
      height: 100vh;
      width: 100%;
      display: flex;
  ...
  `,document.head.appendChild(e)})();w.on(i=>{switch(i.type){case"status":q.setStatus(i.value),S.setState(i.value),i.value==="listening"?z.show():z.hide();break;case"transcript":_.push({speaker:i.speaker,text:i.text,tool_call:i.tool_call});break;case"transcript_start":i.speaker==="assistant"&&_.startAssistant();break;case"transcript_chunk":_.appendAssistant(i.text),q.setStatus("speaking"),S.setState("speaking");break;case"transcript_end":_.endAssistant();break;case"audio_level":S.setRms(i.rms);break;case"config":c=i.cfg,S.setAvatar(c.avatar??"aria");{const e=c.locale??"auto",t=e==="auto"?navigator.language?.split("-")[0]??"en":e;ie(t);const a=c.respond_in==="auto"?c.user_address_language??t:c.respond_in??t;fe(a)}xe(c),c.transparency_acknowledged||ye.show(c.provider,async()=>{const e={...c,transparency_acknowledged:!0};await b.saveConfig(e),c=e}),c.onboarding_seen||P.show(async()=>{const e={...c,onboarding_seen:!0};await b.saveConfig(e),c=e});break;case"toast":me.show(i.level,i.message);break;case"history_replay":_.replayHistory(i.items),ge.populate(i.items);break}});M.onSubmit(({text:i,images:e})=>{b.sendText(i,e.length>0?e:void 0)});M.onRecord(()=>{b.startListening()});q.onSettingsClick(()=>{c&&k.open(c)});k.onSave(async i=>b.saveConfig(i));k.onShowOnboarding(()=>{P.show(async()=>{if(c){const i={...c,onboarding_seen:!0};await b.saveConfig(i),c=i}})});document.addEventListener("keydown",i=>{const e=i.target,t=e&&(e.tagName==="INPUT"||e.tagName==="TEXTAREA"||e.tagName==="SELECT");if((i.metaKey||i.ctrlKey)&&i.key===","){i.preventDefault(),c&&k.open(c);return}if(i.key==="Escape"){k.close();return}if(!t){if(i.key==="/"){i.preventDefault(),document.querySelector("footer [data-input]")?.focus();return}if((i.metaKey||i.ctrlKey)&&i.key==="k"){i.preventDefault(),document.querySelector("footer [data-input]")?.focus();return}}});
