(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))o(s);new MutationObserver(s=>{for(const r of s)if(r.type==="childList")for(const n of r.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&o(n)}).observe(document,{childList:!0,subtree:!0});function t(s){const r={};return s.integrity&&(r.integrity=s.integrity),s.referrerPolicy&&(r.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?r.credentials="include":s.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function o(s){if(s.ep)return;s.ep=!0;const r=t(s);fetch(s.href,r)}})();class H{el;constructor(e){this.el=document.createElement("header"),this.el.className="h-12 px-5 flex items-center justify-between border-b border-border bg-surface/40",this.el.innerHTML=`
    ...
    `,e.appendChild(this.el)}setStatus(e){const t=this.el.querySelector("[data-status]");t&&(t.textContent=e)}onSettingsClick(e){this.el.querySelector("[data-settings]").addEventListener("click",e)}}const _={aria:"/avatars/aria.svg",liam:"/avatars/liam.svg",sage:"/avatars/sage.svg"};class A{el;img;halo;dots;constructor(e,t="aria"){this.el=document.createElement("div"),this.el.className="flex-1 grid place-items-center relative",this.el.innerHTML=`
    ...
    `,e.appendChild(this.el),this.el.querySelector("[data-close]").addEventListener("click",()=>this.close()),this.el.querySelector("[data-cancel]").addEventListener("click",()=>this.close()),this.el.querySelector('[data-field="provider"]').addEventListener("change",()=>{this.refreshSecretFields(),this.refreshModelOptions()}),this.el.querySelector('[data-field="user_address_as"]').addEventListener("change",()=>{this.refreshTitleRow()}),this.el.querySelector("[data-toggle-secret]").addEventListener("click",()=>{const t=this.el.querySelector('[data-field="secret"]'),o=this.el.querySelector("[data-eye-show]"),s=this.el.querySelector("[data-eye-hide]"),r=t.type==="text";t.type=r?"password":"text",o.classList.toggle("hidden",!r),s.classList.toggle("hidden",r)}),this.el.querySelector("[data-save]").addEventListener("click",async()=>{const t=this.read(),o=this.el.querySelector("[data-error]"),s=await this.saveHandler(t);s.ok?(o.classList.add("hidden"),this.close()):(o.textContent=(s.errors||["Save failed"]).join(" · "),o.classList.remove("hidden"))})}open(e){this.populate(e),this.el.classList.remove("translate-x-full")}close(){this.el.classList.add("translate-x-full")}onSave(e){this.saveHandler=e}populate(e){this.currentCfg=e,this.el.querySelector('[data-field="provider"]').value=e.provider,this.el.querySelector('[data-field="hotkey"]').value=e.hotkey,this.el.querySelector('[data-field="roots"]').value=e.allowed_roots.join(", "),this.el.querySelector('[data-field="user_name"]').value=e.user_name||"",this.el.querySelector('[data-field="user_address_as"]').value=e.user_address_as||"none",this.el.querySelector('[data-field="user_title"]').value=e.user_title||"",this.refreshModelOptions(),this.refreshSecretFields(),this.refreshTitleRow(),this.refreshVoiceSection()}refreshVoiceSection(){const e=this.currentCfg?.available_avatars??["aria","liam","sage"],t=this.el.querySelector('[data-field="avatar"]');t.innerHTML="";for(const l of e){const i=document.createElement("button");i.type="button",i.className="rounded-md border-2 border-border p-2 hover:border-accent transition-colors",i.dataset.avatar=l,i.setAttribute("aria-pressed",String(l===(this.currentCfg?.avatar??"aria"))),l===(this.currentCfg?.avatar??"aria")&&i.classList.add("border-accent"),i.innerHTML=`<img src="/avatars/${l}.svg" alt="${l}" class="h-16 w-16 mx-auto rounded-full" />
        <p class="mt-1 text-xs capitalize text-center">${l}</p>`,i.addEventListener("click",()=>{t.querySelectorAll("button").forEach(p=>{p.setAttribute("aria-pressed","false"),p.classList.remove("border-accent"),p.classList.add("border-border")}),i.setAttribute("aria-pressed","true"),i.classList.remove("border-border"),i.classList.add("border-accent")}),t.appendChild(i)}const o=this.currentCfg?.available_voices??[],s=this.el.querySelector('[data-field="voice"]');s.innerHTML="";const r={};for(const l of o)(r[l.language]??=[]).push(l);const n=Object.keys(r).sort((l,i)=>l==="en"?-1:i==="en"?1:l.localeCompare(i));for(const l of n){const i=document.createElement("optgroup"),x=r[l][0].label.match(/\(([^)]+)\)/);i.label=x?x[1]:l==="en"?"English":l;for(const u of r[l]){const g=document.createElement("option");g.value=u.id,g.textContent=u.notes?`${u.label} · ${u.notes}`:u.label,i.appendChild(g)}s.appendChild(i)}this.currentCfg?.voice&&(s.value=this.currentCfg.voice);const b=this.currentCfg?.available_stt_languages??[],d=this.el.querySelector('[data-field="stt_language"]');d.innerHTML="";for(const l of b){const i=document.createElement("option");i.value=l.code,i.textContent=l.label,d.appendChild(i)}this.currentCfg?.stt_language&&(d.value=this.currentCfg.stt_language)}refreshTitleRow(){const e=this.el.querySelector('[data-field="user_address_as"]').value;this.el.querySelector("[data-title-row]").classList.toggle("hidden",e!=="title")}refreshModelOptions(){const e=this.el.querySelector('[data-field="provider"]').value,t=this.el.querySelector('[data-field="model"]'),o=this.currentCfg?.available_models?.[e]??[],s=this.currentCfg?.model;t.innerHTML="";const r=new Set;for(const n of o)r.add(n),t.appendChild(this.makeOption(n));s&&!r.has(s)&&t.appendChild(this.makeOption(s+" (custom)",s)),s&&r.has(s)&&(t.value=s)}makeOption(e,t){const o=document.createElement("option");return o.textContent=e,o.value=t??e,o}refreshSecretFields(){const e=this.el.querySelector('[data-field="provider"]').value,t=this.el.querySelector("[data-secret-label]"),o=this.el.querySelector('[data-field="secret"]'),s=this.el.querySelector("[data-secret-hint]"),r=this.el.querySelector("[data-saved-badge]"),n=this.el.querySelector("[data-eye-show]"),b=this.el.querySelector("[data-eye-hide]");t.textContent=e==="ollama"?"Ollama base URL":"API key",o.value="",o.type=e==="ollama"?"text":"password",n.classList.remove("hidden"),b.classList.add("hidden");const d=e===this.currentCfg?.provider&&this.currentCfg?.has_secret===!0;r.classList.toggle("hidden",!d),e==="ollama"?(o.placeholder=this.currentCfg?.ollama_base_url||"http://localhost:11434",s.textContent="URL of your local Ollama instance."):d?(o.placeholder="✓ key on file — leave blank to keep current",s.textContent="Stored in ~/.voice-assistant/.env, mode 0600. Type a new key only to replace it."):(o.placeholder=e==="anthropic"?"sk-ant-…":e==="openai"?"sk-…":"AIza…",s.textContent="Stored in ~/.voice-assistant/.env, mode 0600.")}read(){const e=this.el.querySelector('[data-field="provider"]').value,t=this.el.querySelector('[data-field="model"]').value.trim(),o=this.el.querySelector('[data-field="hotkey"]').value.trim(),s=this.el.querySelector('[data-field="roots"]').value.split(",").map(T=>T.trim()).filter(Boolean),r=this.el.querySelector('[data-field="secret"]').value.trim(),n=this.el.querySelector('[data-field="user_name"]').value.trim()||null,b=this.el.querySelector('[data-field="user_address_as"]').value,d=this.el.querySelector('[data-field="user_title"]').value.trim()||null,l=this.el.querySelector('[data-field="voice"]'),i=this.el.querySelector('[data-field="stt_language"]'),p=this.el.querySelector('[data-field="avatar"] [aria-pressed="true"]'),x=l?.value||this.currentCfg?.voice||"piper:en_US-amy-medium",u=i?.value||this.currentCfg?.stt_language||"auto",g=p?.dataset.avatar??this.currentCfg?.avatar??"aria",C={provider:e,model:t,hotkey:o,allowed_roots:s,ollama_base_url:e==="ollama"?r||this.currentCfg?.ollama_base_url||"http://localhost:11434":null,user_name:n,user_address_as:b,user_title:d,voice:x,stt_language:u,avatar:g};return e!=="ollama"&&r&&(C._secret=r),C}}class R{el;timer=null;constructor(e){this.el=document.createElement("div"),this.el.className="fixed top-16 left-1/2 -translate-x-1/2 px-4 py-2 rounded-md text-sm font-medium opacity-0 pointer-events-none transition-opacity duration-200 z-40",e.appendChild(this.el)}show(e,t){this.timer!==null&&clearTimeout(this.timer);const o={info:"bg-surface border border-border text-fg",warn:"bg-warn/10 border border-warn/40 text-warn",error:"bg-warn/10 border border-warn/40 text-warn"};this.el.className=`fixed top-16 left-1/2 -translate-x-1/2 px-4 py-2 rounded-md text-sm font-medium opacity-100 transition-opacity duration-200 z-40 ${o[e]}`,this.el.textContent=t,this.timer=window.setTimeout(()=>{this.el.classList.replace("opacity-100","opacity-0")},4e3)}}class B{listeners=[];emit(e){for(const t of this.listeners)t(e)}on(e){return this.listeners.push(e),()=>{this.listeners=this.listeners.filter(t=>t!==e)}}}const y=new B;window.va={emit:a=>y.emit(a)};const c=()=>typeof window<"u"&&typeof window.pywebview<"u",m={startListening:async()=>c()?window.pywebview.api.start_listening():void 0,stopListening:async()=>c()?window.pywebview.api.stop_listening():void 0,sendText:async a=>c()?window.pywebview.api.send_text(a):void 0,getConfig:async()=>c()?window.pywebview.api.get_config():{provider:"anthropic",model:"claude-sonnet-4-6",hotkey:"ctrl+shift+space",allowed_roots:["~"],ollama_base_url:null},saveConfig:async a=>c()?window.pywebview.api.save_config(a):{ok:!0},getHistory:async()=>c()?window.pywebview.api.get_history():[],quit:async()=>c()?window.pywebview.api.quit():void 0};c()?document.addEventListener("pywebviewready",()=>{m.getConfig().then(a=>y.emit({type:"config",cfg:a})),m.getHistory().then(a=>y.emit({type:"history_replay",items:a}))}):setTimeout(async()=>{const a=await m.getConfig();y.emit({type:"config",cfg:a})},100);const h=document.getElementById("app");h.className="h-screen flex flex-col";h.innerHTML="";const k=new H(h),w=new A(h,"aria"),v=new M(h),q=new N(h),S=new O(document.body),P=new R(document.body),E=h.children[2];E.classList.remove("flex-1");E.classList.add("max-h-48","shrink-0");let f=null;y.on(a=>{switch(a.type){case"status":k.setStatus(a.value),w.setState(a.value);break;case"transcript":v.push({speaker:a.speaker,text:a.text,tool_call:a.tool_call});break;case"transcript_start":a.speaker==="assistant"&&v.startAssistant();break;case"transcript_chunk":v.appendAssistant(a.text),k&&k.setStatus("speaking"),w.setState("speaking");break;case"transcript_end":v.endAssistant();break;case"audio_level":w.setRms(a.rms);break;case"config":f=a.cfg,w.setAvatar(f.avatar??"aria");break;case"toast":P.show(a.level,a.message);break;case"history_replay":v.replayHistory(a.items);break}});q.onSubmit(a=>{m.sendText(a)});q.onRecord(()=>{m.startListening()});k.onSettingsClick(()=>{f&&S.open(f)});S.onSave(async a=>m.saveConfig(a));function L(){return document.querySelector("footer [data-input]")}document.addEventListener("keydown",a=>{const e=a.target,t=e&&(e.tagName==="INPUT"||e.tagName==="TEXTAREA"||e.tagName==="SELECT");if((a.metaKey||a.ctrlKey)&&a.key===","){a.preventDefault(),f&&S.open(f);return}if(a.key==="Escape"){S.close();return}if(!t){if(a.key==="/"){a.preventDefault(),L()?.focus();return}if((a.metaKey||a.ctrlKey)&&a.key==="k"){a.preventDefault(),L()?.focus();return}}});
