...
def speaker() -> Speaker:
    """Reuse one Speaker across this file's tests to avoid re-loading the model."""
    return Speaker(voice="en_US-amy-medium")
...
def test_invalid_voice_name_raises_clear_error(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("voice_assistant.tts._CACHE_DIR", tmp_path)
    with pytest.raises(ValueError, match="Invalid piper voice"):
        Speaker(voice="completely_bogus_voice_name_xyz")
